    ...
    setTimeout(async () => {
        try {
            ...
            document.getElementById(
                "khodamName"
            ).innerHTML = `Khodam: <span>${khodam.name}</span>`;
            document.getElementById("khodamMeaning").innerText = khodam.meaning;
            document.getElementById("result").style.display = "flex";
        } catch (err) {
            showPopup(
                "Gagal memuat data! Pastikan file khodam.json sudah ada dan valid."
            );
        } finally {
...
function showPopup(message) {
    ...
    setTimeout(() => {
        ...
        popup.querySelector(".popup-content").style.transform =
